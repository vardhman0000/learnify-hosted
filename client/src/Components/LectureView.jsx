...
const LectureView = () => {
  ...
  if (!lecture) return <div className="text-center p-8">Lecture not found.</div>;
    ...
    <div className="container mx-auto p-8">
      ...
      {lecture.type === 'reading' && (
        <div>
          ...
          <button onClick={handleMarkComplete} className="mt-6 bg-green-500 text-white font-bold py-2 px-4 rounded hover:bg-green-600 transition-colors">
      ...
      {lecture.type === 'quiz' && !quizResult && (
        <form onSubmit={handleSubmitQuiz} className="bg-white p-6 rounded-lg shadow-md">
          {lecture.questions.map((q, index) => (
            <div key={q._id} className="mb-6">
              <p className="font-semibold mb-2">{index + 1}. {q.text}</p>
              <div className="space-y-2">
                {q.answers.map((opt, optIndex) => (
      ...
      {quizResult && (
        <div className="bg-white p-6 rounded-lg shadow-md mt-6">
          ...
          <p className="text-lg">Your score: <span className="font-bold">{quizResult.score} / {quizResult.totalQuestions} ({quizResult.percentage.toFixed(2)}%)</span></p>
          {quizResult.passed ? (
            <div className="mt-4">
              <p className="text-green-600 font-semibold">Congratulations, you passed!</p>
              <button onClick={() => navigate(`/courses/${courseId}`)} className="mt-4 bg-green-500 text-white font-bold py-2 px-4 rounded hover:bg-green-600 transition-colors">
                Continue to Next Lecture
              </button>
            </div>
          ) : (
            <div className="mt-4">
              <p className="text-red-600 font-semibold">You did not pass. Please try again.</p>
              <button onClick={() => { setQuizResult(null); setAnswers({}); }} className="mt-4 bg-red-500 text-white font-bold py-2 px-4 rounded hover:bg-red-600 transition-colors">
                Retry Quiz
              </button>
            </div>
          )}
